...
PurePursuitController::PurePursuitController() : Node("pure_pursuit_controller"), has_center_line_(false)
{
    ...
    this->declare_parameter("kp_lateral", 0.1);
    this->declare_parameter("kp_curve", 0.8);
...
void PurePursuitController::get_parameters()
{
    ...
    this->get_parameter("kp_curve", kp_curve_);
...
geometry_msgs::msg::PointStamped PurePursuitController::findLookaheadPoint()
{
    ...
    // 动态预瞄距离（适配小车尺寸）
    double dynamic_lookahead = lookahead_distance_ + lookahead_gain_ * current_linear_vel_;
...
geometry_msgs::msg::Twist PurePursuitController::calculateControlCommand()
{
    ...
    if (std::abs(denominator) > 1e-6) {
        steering_angle = (2 * y_veh) / denominator;
    }
    ...
    // 计算线速度（弯道减速）
    double linear_speed = max_linear_speed_ * (1.0 - kp_curve_ * std::fabs(steering_angle)/max_angular_speed_);
